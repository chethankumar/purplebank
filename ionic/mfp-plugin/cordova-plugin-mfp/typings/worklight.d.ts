...
declare module WLAuthorizationManager {
	...
	function getAuthorizationServerUrl(): WLPromise;
		...
		/**
	     * @description Sets the authorization flow time out.
	     * if this value is not set by user, then default 10s is considered as the default timeout.
	     * @param {number} timeOut Mandatory. URL of the authorization server to set.
	     * @returns A promise object.
	     * @methodOf WLAuthorizationManager#
	     */
	function setLoginTimeout(timeOut: number): WLPromise;
...
declare module WL.Client {
    ...
    function invokeProcedure(invocationData: Object, options?: Object): WLPromise;
		...
		* @param certificateFilename - name of the certificate file that is located under
		* the certificate folder in the application root
    ...
    function pinTrustedCertificatePublicKey(certificateFilename: string): WLPromise;
        ...
        * Your app must call the certificate pinning method before making a secured request.
        * Calling this method a second time overrides any previous pinning operation.
        * @param certificateFilenames - array of names of certificate file that is located under
        * the certificate folder in the application root
        * @return {Promise} Resolved when the operation is successful and the certificates are pinned. Rejected if any of the certificateFilename is undefined, not found or is not in DER format.
        * @methodOf WL.Client#
        */

    function pinTrustedCertificatePublicKey(certificateFilenames: string[]): WLPromise;
